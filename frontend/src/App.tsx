...
function TodayPage({ config, selectedDate, refresh, onDate, onDataChange, reportError, openHabit, openNote }: {
  ...
  return <section className="page today-page">
    ...
    {activityLogDetail && <ActivityLogDetail activity={activityLogDetail} selectedDate={selectedDate} today={config.today} onClose={() => setActivityLogDetail(null)} onChanged={async () => { await load(); onDataChange(); }} reportError={reportError} />}
  </section>;
}
...
function ActivityLogDetail({ activity, selectedDate, today, onClose, onChanged, reportError }: { activity: ActivityLogDay; selectedDate: string; today: string; onClose: () => void; onChanged: () => Promise<void>; reportError: (error: unknown) => void }) {
...
function HabitDetailView({ habitId, today, onClose, onChanged, onDeleted, reportError, openNote }: { habitId: number; today: string; onClose: () => void; onChanged: () => void; onDeleted: () => void; reportError: (error: unknown) => void; openNote: (target: NoteTarget) => void }) {
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {detailHabitId !== null && <div inert={noteTarget !== null}><HabitDetailView habitId={detailHabitId} today={config.today} onClose={closeHabit} onChanged={refreshAll} onDeleted={closeHabit} reportError={reportError} openNote={openNote} /></div>}
