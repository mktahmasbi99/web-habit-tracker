...
import type { ActivityLogDay, ActivityLogMonth, ActivityLogNote, ActivityLogSummary, ArchivePeriod, BackupFile, BackupSettings, Config, HabitDay, HabitDetail, HabitNote, HabitSummary, NoteDetail, NoteSummary, Status, SystemNotification, TimedActivityDay, TimedActivityNote, TimedActivitySummary, TimedActivityWeek, TimedEntry, Unresolved } from "./lib/types";

type Tab = "today" | "notes" | "manage" | "notifications" | "more";
type Theme = "system" | "arcade" | "crt" | "neon" | "desert" | "cartridge";
...
const themePreference = (): Theme => {
  try { const saved = window.localStorage?.getItem("theme"); return saved === "arcade" || saved === "crt" || saved === "neon" || saved === "desert" || saved === "cartridge" ? saved : "system"; } catch { return "system"; }
};
const saveThemePreference = (theme: Theme) => { try { window.localStorage?.setItem?.("theme", theme); } catch { /* preference storage is optional */ } };
...
function MorePage({ config, reportError, imported, theme, onThemeChange }: { config: Config; reportError: (error: unknown) => void; imported: () => void; theme: Theme; onThemeChange: (theme: Theme) => void }) {
...
export default function App() {
  ...
  return <div className="app-shell">
    <main className="content" inert={detailOpen} aria-hidden={detailOpen || undefined}>
      ...
      {tab === "more" && <MorePage config={config} reportError={reportError} imported={() => { void syncConfig(); }} theme={theme} onThemeChange={setTheme} />}
    </main>
