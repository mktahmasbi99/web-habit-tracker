...
import { InstallAppPanel, type PwaInstallState } from "./components/PwaControls";
import { usePwaInstall } from "./hooks/usePwaInstall";
import { liveTimer, primeTimerAlerts, useTimedActivityTimers } from "./hooks/useTimedActivityTimers";
import { api, ApiError } from "./lib/api";
import type { ActivityLogDay, ActivityLogMonth, ActivityLogNote, ActivityLogSummary, ArchivePeriod, BackupFile, BackupSettings, Config, HabitDay, HabitDetail, HabitNote, HabitSummary, NoteDetail, NoteSummary, Status, SystemNotification, Theme, TimedActivityDay, TimedActivityNote, TimedActivitySummary, TimedActivityTimer, TimedActivityWeek, TimedEntry, TimerMode, Unresolved } from "./lib/types";

type Tab = "today" | "notes" | "manage" | "notifications" | "more";
...
const formatTimerClock = (seconds: number) => {
  const safe = Math.max(0, Math.floor(seconds)); const hours = Math.floor(safe / 3600); const minutes = Math.floor((safe % 3600) / 60); const remainder = safe % 60;
...
function TodayPage({ config, selectedDate, refresh, onDate, onDataChange, reportError, openHabit, openNote }: {
  ...
  const timerTracking = useTimedActivityTimers(selectedDate === config.today, () => { void load(); onDataChange(); }, reportError);
  const timers = new Map(timerTracking.snapshot.timers.map(timer => [timer.activityId, liveTimer(timer, timerTracking.snapshot.serverNow, timerTracking.clock)]));
  ...
  return <section className="page today-page">
    <header className="day-header">
      ...
      <button className="day-title" onClick={event => { event.currentTarget.focus(); setCalendar(true); }}><h1>{title}</h1>{isRelativeDay && <small>{selectedDate}</small>}</button>
    ...
    {!loading && !loadError && loadedDate === selectedDate && <section className="timed-section">
      ...
      {timedOpen && <div id="timed-activity-list">{timed.length === 0 ? <p className="timed-empty">No timed activities on this date.</p> : <div className="habit-list timed-list">{timed.map(activity => { const timer = timers.get(activity.id); return <article className="habit-card timed-card" key={activity.id}><button className="timed-card-open" onClick={() => setTimedDetail({ ...activity, date: selectedDate })} aria-label={`Open ${activity.name}`}><span><strong>{activity.name}</strong><small className="timed-day-total"><Clock3 aria-hidden="true" />{formatMinutes(activity.dayMinutes)}</small>{timer?.mode === "pomodoro" && <span className="pomodoro-card-indicator" aria-label="Pomodoro timer active"><span aria-hidden="true">🍅</span></span>}{timer?.mode === "countdown" && <span className="countdown-card-indicator" aria-label={`${Math.round(timer.percent)} percent complete, ${formatTimerClock(timer.elapsedSeconds)} elapsed, ${formatTimerClock(timer.remainingSeconds)} remaining`}><span><span>{formatTimerClock(timer.elapsedSeconds)} elapsed</span><span>{formatTimerClock(timer.remainingSeconds)} left</span></span><progress max="100" value={timer.percent}>{timer.percent}%</progress></span>}</span><ChevronRight aria-hidden="true" /></button><button className={`note-button ${activity.hasNote ? "has-note" : ""}`} onClick={() => openNote({ habitId: activity.id, habitName: activity.name, date: selectedDate, create: !activity.hasNote, kind: "timed" })} aria-label={`${activity.hasNote ? "View" : "Add"} note for ${activity.name}`}><StickyNote /></button></article>; })}</div>}</div>}
    </section>}
    ...
    {calendar && <Modal title="Calendar" wide onClose={() => setCalendar(false)}><Calendar selected={selectedDate} today={config.today} onSelect={date => { onDate(date); setCalendar(false); }} onJumpToToday={() => { onDate(config.today); setCalendar(false); }} /></Modal>}
    ...
    {timedDetail && <TimedActivityDetail activity={timedDetail} selectedDate={timedDetail.date} today={config.today} timer={timers.get(timedDetail.id) ?? null} onTimerChanged={(timer) => timerTracking.replace(timer, timedDetail.id)} onClose={() => setTimedDetail(null)} onChanged={async () => { await load(); await timerTracking.load(); onDataChange(); }} reportError={reportError} />}
    {activityLogDetail && <ActivityLogDetail activity={activityLogDetail} selectedDate={selectedDate} today={config.today} onClose={() => setActivityLogDetail(null)} onNavigateToDate={date => { setActivityLogDetail(null); onDate(date); }} onChanged={async () => { await load(); onDataChange(); }} reportError={reportError} />}
  </section>;
}

function TimedEntryRow({ activityId, entry, timerActive, onChanged, reportError }: { activityId: number; entry: TimedEntry; timerActive: boolean; onChanged: () => Promise<void>; reportError: (error: unknown) => void }) {
  ...
  return <div className="timed-entry-row">{editing ? <>
    ...
    <div className="timed-entry-actions"><button type="button" onClick={() => setEditing(false)}>Cancel</button><button type="button" className="save" disabled={total < 1 || total > 1440 || (timerActive && total > entry.minutes)} onClick={() => void save()}>Save</button></div>
...
function TimedActivityDetail({ activity, selectedDate, today, timer, onTimerChanged, onClose, onChanged, reportError }: { activity: TimedActivityDay; selectedDate: string; today: string; timer: TimedActivityTimer | null; onTimerChanged: (timer: TimedActivityTimer | null) => void; onClose: () => void; onChanged: () => Promise<void>; reportError: (error: unknown) => void }) {
  const [week, setWeek] = useState<TimedActivityWeek | null>(null); const [hours, setHours] = useState(""); const [minutes, setMinutes] = useState(""); const [saving, setSaving] = useState(false); const [menu, setMenu] = useState(false); const [action, setAction] = useState<"archive" | "restore" | "delete" | null>(null); const [confirmation, setConfirmation] = useState(""); const [activityName, setActivityName] = useState(activity.name); const [editingName, setEditingName] = useState(false); const [nameDraft, setNameDraft] = useState(activity.name); const [entryMode, setEntryMode] = useState<"manual" | TimerMode>("manual");
  ...
  const selected = week?.days.find(day => day.date === selectedDate); const total = durationTotal(hours, minutes); const editable = selectedDate <= today && Boolean(selected?.active); const liveAvailable = selectedDate === today && editable;
  ...
  const startTimer = async () => { setSaving(true); try { await primeTimerAlerts(); const next = await api.startTimer(activity.id, entryMode as TimerMode, entryMode === "countdown" ? total : undefined); onTimerChanged(next); setHours(""); setMinutes(""); await refresh(); } catch (error) { reportError(error); } finally { setSaving(false); } };
  const timerAction = async (operation: () => Promise<TimedActivityTimer>) => { setSaving(true); try { onTimerChanged(await operation()); await refresh(); } catch (error) { const current = error instanceof ApiError && error.status === 409 ? (error.data as { timer?: TimedActivityTimer })?.timer : undefined; if (current) onTimerChanged(current); reportError(error); } finally { setSaving(false); } };
  const cancelTimer = async () => { if (!timer) return; setSaving(true); try { await api.cancelTimer(activity.id, timer.revision); onTimerChanged(null); await refresh(); } catch (error) { const current = error instanceof ApiError && error.status === 409 ? (error.data as { timer?: TimedActivityTimer })?.timer : undefined; if (current) onTimerChanged(current); reportError(error); } finally { setSaving(false); } };
  ...
  return <Modal title={activityName} titleDetail={<span className="timed-title-total">(<Clock3 aria-hidden="true" />{formatMinutes(selected?.minutes ?? activity.dayMinutes)})</span>} titleEditor={editingName ? <label className="modal-title-editor">Activity name<input autoFocus maxLength={200} value={nameDraft} onChange={event => setNameDraft(event.target.value)} /></label> : undefined} wide onClose={onClose} actions={editingName ? <div className="inline-edit-actions"><button className="bar-text-button" onClick={() => { setNameDraft(activityName); setEditingName(false); }}>Cancel</button><button className="bar-text-button save" disabled={!nameDraft.trim() || saving} onClick={() => void rename()}>Save</button></div> : <><button className="icon-button" onClick={() => setEditingName(true)} aria-label="Edit timed activity"><Pencil /></button><div className="menu-anchor"><button className="icon-button" onClick={() => setMenu(value => !value)} aria-label="More timed activity options" aria-expanded={menu}><MoreHorizontal /></button>{menu && <><button className="menu-scrim" aria-label="Close menu" onClick={() => setMenu(false)} /><div className="habit-menu" role="menu">{activity.archived ? <button role="menuitem" onClick={() => { setAction("restore"); setMenu(false); }}>Restore</button> : <button role="menuitem" disabled={Boolean(timer)} title={timer ? "Cancel the live timer first" : undefined} onClick={() => { setAction("archive"); setMenu(false); }}>Archive</button>}<button className="destructive-text" role="menuitem" onClick={() => { setAction("delete"); setMenu(false); }}>Delete</button></div></>}</div></>}><div className="timed-detail">
    <section>{liveAvailable && !timer && <><fieldset className="timer-mode-picker"><legend>Record time</legend>{(["manual", "pomodoro", "countdown"] as const).map(mode => <label className={entryMode === mode ? "selected" : ""} key={mode}><input type="radio" name="timer-mode" checked={entryMode === mode} onChange={() => { setEntryMode(mode); setHours(""); setMinutes(""); }} /><span>{mode === "manual" ? "Manual" : mode === "pomodoro" ? "Pomodoro" : "Countdown"}</span></label>)}</fieldset>{entryMode === "manual" ? <form className="duration-form" onSubmit={event => void add(event)}><DurationFields hours={hours} minutes={minutes} onHoursChange={setHours} onMinutesChange={setMinutes} onNormalize={normalize} /><button className="form-submit" disabled={saving || total < 1 || total > 1440}>Add entry</button></form> : entryMode === "pomodoro" ? <div className="timer-start-panel"><p>Focus for 25 minutes, followed by a 5-minute break. Every fourth break is 15 minutes.</p><button className="form-submit" disabled={saving} onClick={() => void startTimer()}>Start focus</button></div> : <div className="timer-start-panel"><DurationFields hours={hours} minutes={minutes} onHoursChange={setHours} onMinutesChange={setMinutes} onNormalize={normalize} /><button className="form-submit" disabled={saving || total < 1 || total > 1440} onClick={() => void startTimer()}>Start countdown</button></div>}</>}
      {liveAvailable && timer && <div className={`live-timer-panel ${timer.mode}`}><div className="live-timer-heading"><span aria-hidden="true">{timer.mode === "pomodoro" ? "🍅" : "⏳"}</span><div><strong>{timer.mode === "countdown" ? "Countdown" : timer.phase === "focus" ? `Focus ${timer.focusNumber} of 4` : timer.phase === "ready_focus" ? `Focus ${timer.focusNumber} ready` : timer.phase === "long_break" ? "Long break" : "Short break"}</strong><small>{timer.status === "paused" ? "Paused" : timer.status === "ready" ? "Waiting for you" : "Running"}</small></div></div><div className="timer-clock" aria-live="off">{formatTimerClock(timer.remainingSeconds)}</div>{timer.mode === "countdown" && <><progress max="100" value={timer.percent}>{timer.percent}%</progress><div className="timer-progress-labels"><span>{formatTimerClock(timer.elapsedSeconds)} elapsed</span><span>{Math.round(timer.percent)}%</span></div></>}<div className="timer-controls">{timer.mode === "pomodoro" && timer.phase === "focus" && timer.status === "running" && <button className="form-submit" disabled={saving} onClick={() => void timerAction(() => api.pauseTimer(activity.id, timer.revision))}>Pause</button>}{timer.mode === "pomodoro" && timer.phase === "focus" && timer.status === "paused" && <button className="form-submit" disabled={saving} onClick={() => void timerAction(() => api.resumeTimer(activity.id, timer.revision))}>Resume</button>}{timer.mode === "pomodoro" && (timer.phase === "short_break" || timer.phase === "long_break") && <button className="form-submit" disabled={saving} onClick={() => void timerAction(() => api.skipTimerBreak(activity.id, timer.revision))}>Skip break</button>}{timer.mode === "pomodoro" && timer.phase === "ready_focus" && <button className="form-submit" disabled={saving} onClick={() => void timerAction(() => api.startTimerFocus(activity.id, timer.revision))}>Start focus</button>}<button className="timer-cancel" disabled={saving} onClick={() => void cancelTimer()}>Cancel {timer.mode === "pomodoro" ? "Pomodoro" : "countdown"}</button></div></div>}
      {editable && !liveAvailable && <form className="duration-form" onSubmit={event => void add(event)}><DurationFields hours={hours} minutes={minutes} onHoursChange={setHours} onMinutesChange={setMinutes} onNormalize={normalize} /><button className="form-submit" disabled={saving || total < 1 || total > 1440}>Add entry</button></form>}
      <h3>Entries</h3>{timer && <p className="timer-reservation-note">While this timer is active, entries can be deleted or reduced but not added or increased.</p>}{selected?.entries.length ? <div className="timed-entries">{selected.entries.map(entry => <TimedEntryRow key={entry.id} activityId={activity.id} entry={entry} timerActive={Boolean(timer)} onChanged={refresh} reportError={reportError} />)}</div> : <p className="secondary">No time logged. This day remains at zero.</p>}
    </section>
    <section className="timed-statistics"><h3>Weekly statistics</h3><div className="timed-week" aria-label="Week history">{week?.days.map(day => <div key={day.date} className={day.date === selectedDate ? "selected" : ""}><span>{parseDay(day.date).toLocaleDateString("en-GB", { weekday: "short", timeZone: "UTC" })}</span><strong>{formatMinutes(day.minutes)}</strong></div>)}</div><div className="timed-selected-head"><div><span>{prettyDate(selectedDate)}</span><strong>{formatMinutes(selected?.minutes ?? 0)}</strong></div><div><span>Week through this day</span><strong>{formatMinutes(week?.days.filter(day => day.date <= selectedDate).reduce((sum, day) => sum + day.minutes, 0) ?? 0)}</strong></div></div></section>
  </div>{action && <Modal title={action === "archive" ? "Archive timed activity" : action === "restore" ? "Restore timed activity" : "Delete timed activity"} onClose={() => { setAction(null); setConfirmation(""); }}><div className="confirmation-panel">{action === "archive" ? <p>This activity will remain editable through today. Its sessions and notes will be preserved.</p> : action === "restore" ? <p>This activity returns to tracking today. The inactive gap remains unavailable.</p> : <><p>This permanently deletes the activity, all duration entries, notes, and archive history. A safety backup is created first.</p><label>Type <strong>DELETE</strong> to continue<input value={confirmation} onChange={event => setConfirmation(event.target.value)} autoCapitalize="characters" autoFocus /></label></>}<button className={action === "delete" ? "danger-button" : "form-submit"} disabled={saving || (action === "delete" && confirmation !== "DELETE")} onClick={() => void manage()}>{saving ? "Working…" : action === "archive" ? "Archive activity" : action === "restore" ? "Restore activity" : <><Trash2 />Delete permanently</>}</button></div></Modal>}</Modal>;
}

function ActivityLogDetail({ activity, selectedDate, today, onClose, onNavigateToDate, onChanged, reportError }: { activity: ActivityLogDay; selectedDate: string; today: string; onClose: () => void; onNavigateToDate: (date: string) => void; onChanged: () => Promise<void>; reportError: (error: unknown) => void }) {
  const [month, setMonth] = useState(selectedDate.slice(0, 7)); const [data, setData] = useState<ActivityLogMonth | null>(null); const [chosen] = useState(selectedDate); const [working, setWorking] = useState(false); const [menu, setMenu] = useState(false); const [action, setAction] = useState<"archive" | "delete" | null>(null); const [confirmation, setConfirmation] = useState(""); const [activityName, setActivityName] = useState(activity.name); const [editingName, setEditingName] = useState(false); const [nameDraft, setNameDraft] = useState(activity.name);
  ...
  return <Modal title={activityName} titleEditor={editingName ? <label className="modal-title-editor">Activity name<input autoFocus maxLength={200} value={nameDraft} onChange={event => setNameDraft(event.target.value)} /></label> : undefined} wide onClose={onClose} actions={editingName ? <div className="inline-edit-actions"><button className="bar-text-button" onClick={() => { setNameDraft(activityName); setEditingName(false); }}>Cancel</button><button className="bar-text-button save" disabled={!nameDraft.trim() || working} onClick={() => void rename()}>Save</button></div> : <><button className="icon-button" onClick={() => setEditingName(true)} aria-label="Edit activity log"><Pencil /></button><div className="menu-anchor"><button className="icon-button" onClick={() => setMenu(value => !value)} aria-label="More activity log options" aria-expanded={menu}><MoreHorizontal /></button>{menu && <><button className="menu-scrim" aria-label="Close menu" onClick={() => setMenu(false)} /><div className="habit-menu" role="menu"><button role="menuitem" onClick={() => { setAction("archive"); setMenu(false); }}>Archive</button><button className="destructive-text" role="menuitem" onClick={() => { setAction("delete"); setMenu(false); }}><Trash2 />Delete</button></div></>}</div></>}><div className="activity-log-detail"><p className="secondary">{relativeCompletion(activity.lastCompletedDate, today)}</p><div className="calendar"><div className="calendar-head"><button className="icon-button" onClick={() => shift(-1)} aria-label="Previous month"><ChevronLeft /></button><strong>{first.toLocaleDateString("en-GB", { month: "long", year: "numeric", timeZone: "UTC" })}</strong><button className="icon-button" onClick={() => shift(1)} aria-label="Next month"><ChevronRight /></button></div><div className="calendar-grid weekdays">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid days">{Array.from({ length: leading }, (_, index) => <span key={`blank-${index}`} />)}{data?.days.map(day => <button key={day.date} className={`calendar-day activity-log-day ${day.active ? "active" : "inactive"} ${day.date === chosen ? "selected" : ""} ${day.completed ? "completed" : ""} ${day.hasNote ? "has-note" : ""}`} disabled={!day.active || day.date > today} onClick={() => onNavigateToDate(day.date)} aria-label={`${day.date}${day.completed ? ", completed" : ""}${day.hasNote ? ", has note" : ""}`}><span>{Number(day.date.slice(-2))}</span><span className="markers" aria-hidden="true">{day.completed && <i className="done-dot" />}{day.hasNote && <i className="note-dot" />}</span></button>)}</div></div></div>{action && <Modal title={action === "archive" ? "Archive activity log" : "Delete activity log"} onClose={() => { setAction(null); setConfirmation(""); }}><div className="confirmation-panel">{action === "archive" ? <p>This activity remains editable through today. Its completion history and notes are preserved.</p> : <><p>This permanently deletes the activity, its completion history, notes, and archive history. A safety backup is created first.</p><label>Type <strong>DELETE</strong> to continue<input value={confirmation} onChange={event => setConfirmation(event.target.value)} autoCapitalize="characters" autoFocus /></label></>}<button className={action === "delete" ? "danger-button" : "form-submit"} disabled={working || (action === "delete" && confirmation !== "DELETE")} onClick={() => void manage()}>{working ? "Working…" : action === "archive" ? "Archive activity" : <><Trash2 />Delete permanently</>}</button></div></Modal>}</Modal>;
}
...
function ActivityLogManagementDetail({ activity, onClose, onChanged, reportError }: { activity: ActivityLogSummary; onClose: () => void; onChanged: () => void; reportError: (error: unknown) => void }) {
  ...
  return <Modal title={editing ? "Edit activity log" : activity.name} onClose={onClose} actions={!editing && <><button className="icon-button" onClick={() => setEditing(true)} aria-label="Edit activity log"><Pencil /></button><button className="small-button" onClick={() => setAction(activity.archived ? "restore" : "archive")}>{activity.archived ? "Restore" : "Archive"}</button></>}>{editing ? <div className="form"><label>Activity name<input autoFocus value={name} maxLength={200} onChange={event => setName(event.target.value)} /></label><button className="form-submit" disabled={!name.trim() || working} onClick={() => void save()}>Save</button></div> : <div className="detail-facts"><div><span>Type</span><strong>Activity log</strong></div><div><span>Original start date</span><strong>{prettyDate(activity.startDate)}</strong></div><div><span>Saved notes</span><strong>{activity.noteCount}</strong></div><button className="danger-button" onClick={() => setAction("delete")}><Trash2 />Delete permanently</button></div>}{action && <div className="confirmation-panel"><p>{action === "delete" ? "This permanently deletes the activity, its completion history, notes, and archive history. A safety backup is created first." : action === "archive" ? "This activity remains editable through today." : "This activity returns to tracking today."}</p>{action === "delete" && <label>Type <strong>DELETE</strong> to continue<input value={confirmation} onChange={event => setConfirmation(event.target.value)} /></label>}<button className={action === "delete" ? "danger-button" : "form-submit"} disabled={working || (action === "delete" && confirmation !== "DELETE")} onClick={() => void run()}>{action === "delete" ? <><Trash2 />Delete permanently</> : action === "archive" ? "Archive activity" : "Restore activity"}</button></div>}</Modal>;
}
...
function HabitDetailView({ habitId, today, onClose, onNavigateToDate, onChanged, onDeleted, reportError, openNote }: { habitId: number; today: string; onClose: () => void; onNavigateToDate: (date: string) => void; onChanged: () => void; onDeleted: () => void; reportError: (error: unknown) => void; openNote: (target: NoteTarget) => void }) {
  const [habit, setHabit] = useState<HabitDetail | null>(null); const [editing, setEditing] = useState(false); const [name, setName] = useState(""); const [startDate, setStartDate] = useState(""); const [menu, setMenu] = useState(false); const [action, setAction] = useState<"archive" | "restore" | "delete" | null>(null); const [confirmation, setConfirmation] = useState(""); const [working, setWorking] = useState(false); const [history, setHistory] = useState(false);
  const [loadError, setLoadError] = useState("");
  const [attempt, setAttempt] = useState(0);
  useEffect(() => {
    let active = true; setLoadError("");
    void api.habitDetail(habitId).then(value => { if (active) { setHabit(value); setName(value.name); setStartDate(value.startDate); } }).catch(error => { if (active) { setLoadError("Could not load habit."); reportError(error); } });
    return () => { active = false; };
  ...
  const update = async () => { setWorking(true); try { const value = await api.updateHabit(habit.id, name, startDate); setHabit(value); setName(value.name); setStartDate(value.startDate); setEditing(false); onChanged(); } catch (error) { reportError(error); } finally { setWorking(false); } };
...
function MorePage({ config, reportError, imported, theme, onThemeChange, pwaInstall }: { config: Config; reportError: (error: unknown) => void; imported: () => void; theme: Theme; onThemeChange: (theme: Theme) => void; pwaInstall: PwaInstallState }) {
...
export default function App() {
  ...
  const syncConfig = useCallback(async () => {
    ...
    try {
      ...
      setConfig(value); setTheme(value.theme); setError(""); setOnline(true);
      if (dayChanged) refreshAll(); else void refreshNotifications().catch(reportError);
    } catch (error) { setOnline(false); reportError(error); }
  ...
  const changeTheme = useCallback(async (nextTheme: Theme) => {
    ...
    setTheme(nextTheme);
    try { setTheme((await api.saveTheme(nextTheme)).theme); }
    catch (error) { setTheme(previousTheme); reportError(error); }
  }, [theme, reportError]);
  ...
  if (!config || !selectedDate) return <main className="startup"><img src="/app-icon.png" alt="" /><h1>Habit Tracker</h1><p role="status">{!online ? "You’re offline. Reconnect to the server, then retry." : error || "Opening your habits…"}</p>{(error || !online) && <button className="form-submit" onClick={() => void syncConfig()}>Retry</button>}</main>;
  ...
  return <div className="app-shell">
    <main className="content" inert={detailOpen} aria-hidden={detailOpen || undefined}>
      ...
      {tab === "more" && <MorePage config={config} reportError={reportError} imported={() => { void syncConfig(); }} theme={theme} onThemeChange={nextTheme => { void changeTheme(nextTheme); }} pwaInstall={pwaInstall} />}
    </main>
    ...
    {!online && <div className="offline-alert" role="status">Offline — reconnect to the server to refresh or save changes.</div>}
    {detailHabitId !== null && <div inert={noteTarget !== null}><HabitDetailView habitId={detailHabitId} today={config.today} onClose={closeHabit} onNavigateToDate={date => { openDate(date); closeHabit(); }} onChanged={refreshAll} onDeleted={closeHabit} reportError={reportError} openNote={openNote} /></div>}
